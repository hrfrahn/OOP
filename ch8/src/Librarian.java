...
class Librarian{
	...
	//FIXME: prints books even when they are checked back in
		for(int i = 0; i < pats; i++){
			System.out.println("\nPatron "+(i+1));
			...
			for(int x = 0; x < 3; x++){
				if((!(patrons[i].getBook(x).getTitle().equals("<default>")))||patrons[i].getBook(x).isCheckedOut()==true){
					...
					Book test = patrons[i].getBook(x);
					System.out.println(test.isCheckedOut());	
					System.out.println(patrons[i].getBook(x).isCheckedOut());
				}		
			}
		}
	}
